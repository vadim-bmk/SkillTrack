package com.dvo.progress_service.web.model.handler;

import com.dvo.progress_service.exception.EntityExistsException;
import com.dvo.progress_service.exception.EntityNotFoundException;
import com.dvo.progress_service.web.model.response.ErrorResponse;
...
public class ExceptionControllerHandler {
    ...
    public ResponseEntity<ErrorResponse> handleMalformedURLException(MalformedURLException ex) {
        log.warn("Некорректный URL: {}", ex.getMessage());

        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse("Некорректный URL"));
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<ErrorResponse> handleHttpMessageNotReadableException(HttpMessageNotReadableException ex) {
        log.warn("Некорректный URL ресурс: {}", ex.getMessage());

        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse("Некорректный URL ресурс"));
    }
}
